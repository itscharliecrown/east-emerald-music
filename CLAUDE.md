...
**2026-09-14 (later):** Melody generator shipped and verified (`engine/compose/melody.py`, request kind `melody`, `POST /v1/loops/{id}/melody`). Claude writes notes as scale degrees; `enforce()` snaps strong beats to chord tones, caps range at 19 st, breaks 4th repeats, guarantees rests, ends on a chord tone. Harmony gate for melodies is looser (mean ≥ 0.55). Next asks from Charlie: import own sample (detect key/BPM, user confirms chords), more UI polish.

**2026-09-14:** Everything below verified live. Request kinds: `prompt`, `composed`, `midi` (CPU `midi_job`, no GPU), `variation` (parent loop → a2a, no Claude), `companion` (`drums` = prompt-mode drum loop, gates skip key; other kinds = composed in the parent's key/BPM), `adjust` (parent context + instruction). `requests.session_id` groups a song idea; `/v1/sessions` + zip download. DB `mode` column only accepts the original five; request kinds map to `prompt` at ingestion. `loops.category` allows `drums` (migration 003 rebuilt the table). UI on Vercel: `web/` root dir, static export.
